function tsnegmmMatchSWFlies(jobTag)

% tsnegmmMatchSWFlies(jobTag)
% Run t-SNE GMM clustering on all of our flies, take k from previous PCA20 GMM sparse watershed mapping run, use lockfiles for synchronization
%
% Inputs:
...
parfor iParIndex=1:numCores
    ...
    while foundJob
        ...
        for iFly=randperm(length(flies))
            ...
            % Load this fly's t-SNE mapping results, we process its embeddingValues below
            ...
            pathResult=sprintf('~/results/%s/%s_tsne2gmm_%s_%d.mat',jobTag,jobTag,flyName,k);
