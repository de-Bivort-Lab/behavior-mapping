function pcagmmMatchTMFlies(jobTag)

% pcagmmMatchTMFlies(jobTag)
% Run PCA-GMM clustering on all of our flies, take k from previous t-SNE mapping run, use lockfiles for synchronization
%
% Inputs:
...
parfor iParIndex=1:numCores
    ...
    while foundJob
        ...
        for iFly=randperm(length(flies))
            ...
            pathResult=sprintf('~/results/%s/%s_pcagmm_%s_%d.mat',jobTag,jobTag,flyName,k);
